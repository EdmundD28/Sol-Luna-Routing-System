...
- For a same-Luna implementation and repair loop with a non-overlapping Sol acceptance lane, validate the event sequence with `scripts/closure_contract.py`; it rejects writeful Sol acceptance and reports fine-grained accepted versus reclaimed Luna baseline weight. It is a contract checker, not a dispatcher.
