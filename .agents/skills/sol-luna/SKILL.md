...
Keep this evidence in the task report by default. Do not persist raw prompts, private paths, task identifiers, logs, or cross-repository history unless the user separately asks for a ledger. If a persistent ledger is requested, redact sensitive content, keep it repository-local and ignored where practical, and separate measured facts from judgments. Do not infer economic superiority from one run, incomparable task families, self-authored tests alone, or a displayed allowance percentage whose accounting basis is unknown.
