...
- For a same-Luna implementation and repair loop with a non-overlapping Sol acceptance lane, use `scripts/closure_contract.py`; `project` turns a frozen live prefix into the next legal event and exact repair targets so Sol need not reinterpret the full history. It remains replay-only and never dispatches work.
