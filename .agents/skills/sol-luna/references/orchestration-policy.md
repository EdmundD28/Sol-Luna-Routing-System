...
Validation separation is risk-adaptive. For a clean low-risk package, Luna runs targeted checks and one final authoritative suite, then Sol checks the content-addressed candidate summary, causal mapping, receipt completeness, and environment/scope gaps. For standard/deep, shared-interface, repaired, or causally uncertain work, Luna first supplies targeted evidence; Sol performs focused causal review before the expensive final suite; new failures and changed boundaries return to the same Luna for targeted repair; only after causal closure does that Luna run one authoritative suite on the final candidate. A final-suite failure returns its exact new failures to that Luna. Sol never repeats the full suite by default. It runs only the smallest check activated by drift, incomplete evidence, nondeterminism, failed Sol verification, security, or platform risk. If Luna lacks the authoritative environment, Luna does causal/targeted checks and Sol runs that environment once. The external common referee remains outside measured route intervals.
...
For complete-Luna validation, `CAUSAL`, `PRE_FINAL_CAUSAL`, `RETURN_TO_LUNA`, and `FALLBACK` are `validation_mode` values, while `review_depth` retains the compatible `TARGETED`/`STANDARD`/`DEEP` enum. `PRE_FINAL_CAUSAL` places focused Sol causal review before Luna's one final suite; `CAUSAL` verifies its final receipt; `RETURN_TO_LUNA` covers low-risk missing evidence or an exact Luna suite failure; and `FALLBACK` means Luna cannot access the authoritative environment (Sol runs that full suite once). Concrete risk triggers take precedence over generic high risk.

The reasoning floor permits `medium` for settled, deterministic profiles with up to two cheaply tested signals; unsettled, nondeterministic, high-coupling, or larger-signal profiles retain higher floors so cost reduction cannot bypass quality evidence or accepted-delivery gates.
