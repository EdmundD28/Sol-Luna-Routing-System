...
The compact protocol is a projection: write/read one frozen manifest, use `RUN`/`OK` for normal messages, and expand only `BLOCK`; it does not replace ownership or acceptance authority.
