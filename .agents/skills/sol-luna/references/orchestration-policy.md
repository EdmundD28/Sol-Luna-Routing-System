...
The compact protocol is a projection: freeze the full manifest once in a shared artifact, start Luna with `fork_turns="none"`, and send/read only its `MAN|<package_ref>` reference before `RUN`/`OK`; never paste the manifest body into model messages. Focused repair reuses the same Luna and transmits only new failure evidence and changed boundaries. Only `BLOCK` expands; the protocol does not replace ownership or acceptance authority.
