...
For complete-Luna validation, `CAUSAL`, `RETURN_TO_LUNA`, and `FALLBACK` are `validation_mode` values, while `review_depth` retains the compatible `TARGETED`/`STANDARD`/`DEEP` enum. `CAUSAL` follows Luna's final authoritative-suite receipt, `RETURN_TO_LUNA` covers missing evidence, and `FALLBACK` means Luna cannot access the authoritative environment (Sol runs that full suite once); risk-triggered `DEEP` remains applicable, with concrete triggers taking precedence over generic high risk.
