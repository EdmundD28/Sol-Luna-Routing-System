...
Before execution, callers may pass a closed task image to `select_adaptive_route`. It records modalities, output, acceptance, coupling, risk, size, and a closed capability table whose operation, actor, and surface must be reachable. Unknown capability or missing role binding selects `S0`; unproven capability may be `S4` only for budgeted research. Production S1/S2/S3 requires matching task-family evidence, the conservative Wilson quality bound, complete credit economics, and strict elapsed improvement; High cannot bypass those gates. Effort is a finite heuristic over distinct complexity, tool-operation, and verification-cost signals, not a claim of optimality. Cold-start economics uses `C=L+O+qR`; `q <= (0.5B-L-O)/R` is exact (no solution when `L+O > 0.5B`). Validate the returned execution token with `validate_adaptive_execution`; mismatches fail closed.

Formal exploration may add an optional `difficulty_profile` containing only pre-execution reasoning stages, constraint interactions, semantic ambiguities, and the source of each fact group. Its D1/D2/D3 band is descriptive and never maps mechanically to Low/Medium/High; matching history must carry the exact profile fingerprint. A preregistered Medium/High counterfactual remains S4 research and binds experiment ID, task and acceptance digests, profile, effort, and finite time/attempt budgets into the execution token. A declared budget is not proof of runtime compliance: `validate_adaptive_execution` reports it as unverified unless a matching receipt supplies observed elapsed time and attempts, and it does not cancel models automatically. Ordinary requests need none of these research fields.
...
An ordinary one-writer dispatch contains the repository root, package ID, deliverable, exclusive writable paths, named read dependencies, a launch-ready acceptance capsule, forbidden actions, stop conditions, and effort. This is a dispatch contract, not runtime enforcement: it states the canonical repository workdir, exact executable plus arguments, and environment values already resolved from that workdir, and instructs Luna to execute there without `cd` or reinterpretation. A missing, inconsistent, or unexecutable capsule before launch returns `BLOCK|<package>|K=ACCEPTANCE_CAPSULE|REF=<minimal>`; an exact launch that fails before tests collect returns infrastructure `FAILED` with minimal launch evidence. Neither is quality evidence or authorizes effort escalation; retry only with a corrected capsule, not an implementation repair. During that initial read, `create`, `add`, `update`, and `must cover` clauses freeze required paths and coverage labels; merely allowed paths do not. Luna maps each label to `test-file::test-id`, checks path and ID presence, and fixes omissions. If those tests are pre-existing and independent, Luna runs the sole final suite and Sol stays receipt-only. If Luna creates or changes them, Luna returns after causal checks; Sol reviews only those changed test bodies for real label semantics, rejects empty, assertion-free, or tautological tests, then runs the sole final suite. Price this Sol work before routing. A discrepancy sends only missing labels and minimal evidence to the same Luna. Do not attach route estimates, manifests, ledgers, digests, or tool manuals unless needed for the actual change.
...
Validation separation: name one in-route executor for every suite. Luna causally smokes changed public boundaries and uses the cheapest permitted in-memory structural gate. Refresh only affected checks after candidate changes. With independent existing acceptance, Luna runs the final suite once and Sol reviews ownership, paths, mapping, risk, host results, and any formal receipt without repeating checks. With Luna-authored acceptance, Luna returns before the suite; Sol reviews only changed test semantics and runs the final suite once. If Luna lacks the authoritative environment or its result is not host-observable, Sol likewise becomes the sole executor. After failure Luna returns `FAILED` and waits for exact new evidence. One repair may address every local failure without changed architecture, context, or ownership; failure count alone never forces reclaim. Drift, incomplete evidence, nondeterminism, failure, repair, or material security/platform risk activates only the smallest affected check. The external common referee remains outside measured route intervals.
