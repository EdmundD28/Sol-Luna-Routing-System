...
Complete-Luna validation assigns each suite one in-route executor. For semantic refactors Luna runs one smallest causal smoke per changed public boundary, then an affected in-memory syntax or schema preflight before the expensive suite; neither emits files. Direct imports require explicit contract permission. Refactors move live behavior into intended modules instead of hiding, duplicating, or compressing it to game line caps, and retain a public compatibility facade only when acceptance requires it. The initial task read freezes explicit required-change paths and required coverage labels, distinct from merely allowed paths. In the same turn and before the final suite, Luna maps each label to `test-file::test-id`, checks required paths and named-ID presence, and fixes omissions. Its compact final handoff carries the mapping; Sol confirms only changed paths and named IDs after return, without reading test bodies or repeating the suite. A discrepancy sends only missing paths or labels to the same Luna as exception evidence. Luna returns immediately after a pass. A failure returns `FAILED`; only Sol's new evidence authorizes repair and a triggered rerun. One bounded repair may cover every exact failure from that suite when the fixes stay inside the frozen ownership and need no architecture or context expansion; failure count alone does not force reclaim. Sol reviews ownership, paths, causal coverage, and risks without repeating Luna's checks. If Luna cannot access the authoritative environment or its test result is not host-observable, Sol is the sole full-suite executor. The common external referee remains outside route intervals.
