...
- Luna-owned critical-path work may select High, XHigh, or Max only after the same-allocation lower-effort option is rejected by a quality or defect gate.
...
See the machine-readable [attempt index](docs/benchmark/attempt-index.json) before proposing an upgrade, plus [P016](docs/benchmark/p016-complete-luna-allowance-2026-08-29.md), [the p004 rolling-context campaign](docs/benchmark/matched-allowance-p004-2026-08-28.md), [the p003 allowance campaign](docs/benchmark/matched-allowance-p003-2026-08-28.md), [the p002 allowance pilot](docs/benchmark/matched-allowance-p002-2026-08-28.md), [the matched bounded-function campaign](docs/benchmark/matched-bounded-campaign-2026-08-26.md), and the [older preliminary comparison](docs/benchmark/preliminary-comparison.md). The guarded release command refuses a tag without one exact release entry, prior-family references, and—when retrying an old direction—a concrete changed premise. This history gate is release-only and never enters normal dispatch.
...
Complete-Luna validation assigns each suite one in-route executor. When Sol has no path-disjoint work, it waits once on the retained Luna until completion; periodic status polling, global agent enumeration, and commentary-triggered wakeups are not ordinary work. Luna performs causal checks and cheap in-memory structural preflight while implementing. The initial read freezes explicit required paths and coverage labels, distinct from merely allowed paths, and maps them to named tests. When those tests are pre-existing and independent, Luna runs the final suite once and Sol checks only paths, mapping, risk, and the host result. When Luna creates or changes designated acceptance tests, they cannot certify themselves: Luna returns after causal checks, Sol reviews only those changed test bodies against the frozen labels, rejects empty, assertion-free, or tautological tests, and runs the sole final suite. This Sol cost is priced before dispatch; an uneconomic route stays `SOL_ONLY`. A discrepancy sends only missing labels and minimal evidence to the same Luna. Candidate changes invalidate affected checks; after a pass there are no duplicate rereads or tests. A failure returns `FAILED`; only Sol's new evidence authorizes bounded repair. The common external referee remains outside route intervals.

The normal Luna handoff is a short human-readable `READY`, `BLOCK`, or `FAILED` line backed by the repository diff and named checks. The worker is not required to discover a protocol command, construct a manifest, or compute digests. The dependency-closed manifest and compact receipt tools remain available only for formal measurement, disputed ownership, or high-risk evidence. Exceptions send only new evidence to the same Luna.

When a formal campaign requires canonical receipts, Sol supplies the exact `python .agents/skills/sol-luna/scripts/compact_protocol.py receipt ...` command; ordinary delivery does not pay this cost.
