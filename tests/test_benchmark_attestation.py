...
class BenchmarkAttestationTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        minute = 1
        remaining = 100
        for first in ("SOL_ONLY", "SOL_LUNA", "SOL_LUNA", "SOL_ONLY"):
            for route in (first, "SOL_LUNA" if first == "SOL_ONLY" else "SOL_ONLY"):
                ...
                remaining -= 1
                ATTEST.CAMPAIGN.begin_arm(self.ledger, route=route, route_revision="v0.6.0",
                ...
                remaining -= 1
                ATTEST.CAMPAIGN.end_arm(self.ledger, route=route, observed_at=end.isoformat(),
                ...
                minute += 3
        self.contract.write_text(json.dumps({"schema_version": 1, "campaign_id": "campaign-001",
            ...
            "route_revision": "v0.6.0", "expected_pairs": 4, "expected_batch_size": 1,
            "expected_sol_luna_effort": "medium", "expected_sol_luna_writer_count": 1}, indent=2) + "\n", encoding="utf-8")
        runs = []
        for pair in range(1, 5):
            for route in ("SOL_ONLY", "SOL_LUNA"):
                ...
                workers = [] if route == "SOL_ONLY" else [identity("gpt-5.6-luna", "medium", 100 + pair)]
                runs.append({"pair_id": f"pair-{pair:03d}", "route": route, "controller": controller, "workers": workers})
        content = {"schema_version": 2, "campaign_id": "campaign-001", "sol_luna_effort": "medium", "sol_luna_writer_count": 1, "runs": sorted(runs, key=lambda x: (x["pair_id"], x["route"])), "verification_status": "verified"}
    ...
    def test_happy_path_is_deterministic_and_redacted(self) -> None:
        ...
        self.assertEqual(first["completed_pairs"], 4)
        self.assertEqual(first["arm_count"], 8)
    ...
    def test_digest_prefix_and_float_integer_values_are_strict(self) -> None:
        ...
        contract["expected_sol_luna_writer_count"] = 1.0
        self.contract.write_text(json.dumps(contract), encoding="utf-8")
        with self.assertRaises(ATTEST.AttestationError):
            ATTEST.build_attestation(self.ledger, self.index, self.contract)
    ...
    def test_duplicate_nonfinite_and_unhashable_values_fail_without_traceback(self) -> None:
        ...
        with self.assertRaises(ATTEST.AttestationError):
            ATTEST.build_attestation(self.ledger, self.index, self.contract)
        contract = json.loads(json.dumps({"schema_version": 1, "campaign_id": "campaign-001",
            "benchmark_contract_digest": DIGEST, "task_spec_digest": "sha256:" + "c" * 64,
            "acceptance_suite_digest": "sha256:" + "d" * 64, "policy_fingerprint": "sha256:" + "e" * 64,
            "route_revision": "v0.6.0", "expected_pairs": 4, "expected_batch_size": 1,
            "expected_sol_luna_effort": "medium", "expected_sol_luna_writer_count": 1}))
